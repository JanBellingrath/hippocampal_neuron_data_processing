...
remy = Animal('local2/Jan/Remy/Remy/remy', 'remy') # it only said "rem" here, but am pretty sure it should be remy
...
def get_task(animal):
    ...
    if animal == frank or animal.directory == "fra":
        ...
        #print("detected frank as animal")
        
    elif animal == egypt or animal.directory == "egy":
        task_files = glob(join(animal.short_name, 'egytask*.mat'))
        
    elif animal == government or animal.directory == "gov":
        task_files = glob(join(animal.short_name, 'govtask*.mat'))
        
    elif animal == remy or animal.directory == "remy":
        task_files = glob(join(animal.short_name, 'remytask*.mat')) # here too! Must add y to rem everywhere!!

    elif animal == government or animal.directory == "Fiv":
        task_files = glob(join(animal.short_name, 'Fivtask*.mat'))
        print("animal:", animal)
