...
def get_spike_data(conjoined_key_epochs_per_day_dict, time_len_splitted_chunks, area, animal, animals_dict):
    '''
    ...
    spike_dict = behav.coarse_grained_spike_generator_dict(conjoined_key_epochs_per_day_dict, animals_dict)
    
    neuron_ids = neuron_ids_for_specific_animal_and_subarea(area, animal, animals_dict)

    #getting the time index of the neuronal data
    time_dict = sdd.time_index_dict(neuron_ids, animals_dict)  
...
def neuron_ids_for_specific_animal_and_subarea(area, animal, animals_dict):
    ...
    splitted_epoch_dataframe = divis.split_neuron_dataframe_informationally(task.make_epochs_dataframe(animals_dict[animal]),['type'])
